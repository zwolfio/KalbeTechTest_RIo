        ...
        <table className="w-full border text-sm border-gray-300 ">
          <thead className="sticky top-0 bg-green-600 text-white">
            <tr>
              ...
              <th className="p-2 text-left">Module Flow</th>
              <th className="p-2 text-left">Mandays</th>
          ...
          <tbody>
            {data?.map((row, idx) => (
              <tr key={idx} className="border-b  border-gray-300 hover:bg-gray-50">
                <td className="p-2">
                  <Link href={`/master/tracker/${row.id}`} className="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">
                    Edit
                  </Link></td>
                ...
                <td className="p-2">{row.isUsingNotification ? "✔️" : "❌"}</td>
              </tr>
            ))}
          </tbody>
        </table>
